...
public interface BookBorrowersRepository extends JpaRepository<BookBorrower, Long> {
    ...
    @Query(value = "select * from Books_Borrowers where book_id = :bookId and is_returned = false", nativeQuery = true)
    Optional<BookBorrower> getByBookId(@Param("bookId") Long bookId);
